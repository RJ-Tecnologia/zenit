import { MoreHorizontalIcon, PieChartIcon } from 'lucide-react'
...
export function OutcomeByCategoryCard({
  categoriesSummary
  ...
  return (
    <Card className="bg-white/5 backdrop-blur-xl border-white/8 rounded-2xl shadow-xl overflow-hidden">
      <CardHeader className="flex flex-row items-center justify-between">
        ...
        <MoreHorizontalIcon className="size-5 text-muted-foreground stroke-[2px] cursor-pointer" />
      </CardHeader>
      <CardContent>
        {hasData ? (
          <div className="space-y-6">
            {categoriesSummary.map((category) => (
              <div key={category.name} className="flex items-center group">
                <div className="size-10 rounded-xl bg-error/10 flex items-center justify-center text-error border border-error/20 mr-4 group-hover:bg-error/20 transition-colors">
                  <PieChartIcon className="size-5 stroke-[2px]" />
                </div>
                ...
                <div className="ml-6 text-right">
                  <div className="text-body-md font-bold text-error">
          ...
          <div className="py-12 flex flex-col items-center justify-center text-center">
            <div className="size-16 rounded-full bg-white/5 flex items-center justify-center mb-4 border border-white/8">
              <PieChartIcon className="size-8 text-muted-foreground/50 stroke-[1.5px]" />
            </div>
            <p className="text-body-md font-semibold text-on-surface mb-1">
              Nenhum dado de despesa disponível
            </p>
            <p className="text-sm text-muted-foreground max-w-[250px]">
              Adicione transações de saída para visualizar o detalhamento por
              categoria.
            </p>
          </div>
        )}
